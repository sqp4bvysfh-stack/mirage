...
export async function handleBoostInteraction(
  ...
  // ── Formulaire soumis : envoyer la demande ───────────────
  if (
    ...
    const nom = interaction.fields
      .getTextInputValue("rp_nom")
      .trim();

    const couleur = interaction.fields
      .getTextInputValue("rp_couleur")
      .trim()
      .toUpperCase();

    const emoji = interaction.fields
      .getTextInputValue("rp_emoji")
      .trim();

    if (!/^#[0-9A-F]{6}$/.test(couleur)) {
      await interaction.editReply(
        "❌ Couleur invalide. Utilise un code hex comme `#FF73FA`.",
      );
      return;
    }
    ...
    const demandeChannel =
      await interaction.guild.channels
        .fetch(BOOST_REQUEST_CHANNEL_ID)
        .catch(() => null);
    ...
    const colorInt = Number.parseInt(
      couleur.slice(1),
      16,
    );
...
// ─── Commande *boostsetup ──────────────────────────────────
// Conservée pour compatibilité, mais les IDs sont fixes.
