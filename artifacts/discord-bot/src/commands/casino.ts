...
export const rouletteCommand: Command = {
  ...
  usage: "&roulette <montant> <rouge|noir|pair|impair|0-36>",
  execute: async (message, args) => {
    ...
    if (isNaN(montant) || montant <= 0 || !choix) {
      await message.reply("❌ Usage : `&roulette <montant> <rouge|noir|pair|impair|0-36>`"); return;
    }
    const u = getUser(guildId, userId);
    if (u.poche < montant) { await message.reply(`❌ Poche insuffisante (${fmt(u.poche, cfg.monnaie)}).`); return; }
...
export const blackjackCommand: Command = {
  ...
  execute: async (message, args) => {
    ...
    const cfg     = getEcoConfig(guildId);
    const montant = parseInt(args[0] ?? "");
    if (isNaN(montant) || montant <= 0) { await message.reply("❌ Usage : `&blackjack <montant>`"); return; }
    const u = getUser(guildId, userId);
    if (u.poche < montant) { await message.reply(`❌ Poche insuffisante (${fmt(u.poche, cfg.monnaie)}).`); return; }
    ...
    collector.on("collect", async (i: Interaction) => {
      ...
      if (id === "bj_tirer" || id === "bj_doubler") {
        ...
        if (sj === 21 || id === "bj_doubler") {
          // Auto-rester si 21 ou après doublage
